...
class TERBouyomiCog(TERBaseCog):
    ...
    def __init__(self,
        ...
        self.__ignores_sender_name_suffix_num: bool = bool(
            self.__settings_replaced['limitsWhenPassing']['ignoresSenderNameSuffixNum']
        )
        #   送信ユーザーのユーザー名ないし表示名の、先頭からの文字数の上限
        self.__num_sender_name_characters: int = int(
            self.__settings_replaced['limitsWhenPassing']['numSenderNameCharacters']
        )
        #   先頭からのエモート(スタンプ)数の上限
    ...
    async def message_response(self, message: Message) -> None:
        ...
        sender_user_name = sender_user_name[:self.__num_sender_name_characters]
        sender_display_name = sender_display_name[:self.__num_sender_name_characters]
        #
        #
        # 置換される文字列たちを定義
