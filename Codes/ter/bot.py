...
from .cogs import TERBouyomiCog, TERRaidCog, TERTransCog
...
class TERBot(commands.Bot):
    ...
    async def event_ready(self) -> None:
        print("  Making bot ready ...")
        assert self.user_id is not None, "Bot user ID is unknown."
        print(f"    Bot user ID = {self.user_id}")
        assert self.nick is not None, "Bot user name is unknown."
    ...
    async def __kill(self, ctx: commands.Context):
        if self.__is_by_channel_broadcaster_or_myself(ctx) is True:
            ...
            if is_valid_return_code is False:
                print("      * Invalid return code input")
            print("  done.")
            print("")
